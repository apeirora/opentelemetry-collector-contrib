...
func NewCertificateReaderFromK8sSecretForVerification(ctx context.Context, config *K8sSecretConfig, logger interface{}) (*CertificateReader, error) {
	var zapLogger *zap.Logger
	if l, ok := logger.(*zap.Logger); ok {
		zapLogger = l
	}
	...
	return parseCertificateForVerification(certPEM)
}
...
func parseCertificateForVerification(certPEM []byte) (*CertificateReader, error) {
	if len(certPEM) == 0 {
		return nil, fmt.Errorf("certificate data is empty")
	}
