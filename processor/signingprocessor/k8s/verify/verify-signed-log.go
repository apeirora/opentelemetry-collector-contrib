...
func parseLogRecords(raw []byte) ([]LogRecord, error) {
	...
	if _, hasResourceLogs := top["resourceLogs"]; hasResourceLogs {
		...
		resourceLogs, _ := top["resourceLogs"].([]any)
		for _, rl := range resourceLogs {
			rlMap, _ := rl.(map[string]any)
			scopeLogs, _ := rlMap["scopeLogs"].([]any)
			for _, sl := range scopeLogs {
				slMap, _ := sl.(map[string]any)
				lrs, _ := slMap["logRecords"].([]any)
				for _, lr := range lrs {
	...
	if err := json.Unmarshal(raw, &record); err != nil {
		return nil, errors.New("could not parse log data as OTLP format or single record")
	}
