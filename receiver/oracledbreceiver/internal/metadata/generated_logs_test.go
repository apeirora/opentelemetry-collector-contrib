...
func TestLogsBuilder(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			allEventsCount++
			lb.RecordDbServerSessionWaitSampleEvent(ctx, timestamp, "oracledb.sid-val", "oracledb.serial-val", "oracledb.event-val", "oracledb.wait_class-val", 19, 22.100000, "db.namespace-val")
			...
			for i := 0; i < lrs.Len(); i++ {
				...
				case "db.server.query_sample":
					...
					attrVal, ok = lr.Attributes().Get("oracle.db.service")
					assert.True(t, ok)
					assert.Equal(t, "oracle.db.service-val", attrVal.Str())
					...
					attrVal, ok = lr.Attributes().Get("oracledb.query.wait_time")
					assert.True(t, ok)
					...
					attrVal, ok = lr.Attributes().Get("oracledb.procedure_name")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.procedure_name-val", attrVal.Str())
					...
					attrVal, ok = lr.Attributes().Get("oracledb.query.started")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.query.started-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("oracledb.session.started")
					assert.True(t, ok)
					assert.Equal(t, "oracledb.session.started-val", attrVal.Str())
				...
				case "db.server.session.wait_sample":
					...
					assert.True(t, ok)
					assert.Equal(t, "oracledb.wait_class-val", attrVal.Str())
					attrVal, ok = lr.Attributes().Get("oracledb.wait.count")
					assert.True(t, ok)
				...
				case "db.server.top_query":
					...
					attrVal, ok = lr.Attributes().Get("oracledb.command_type")
					assert.True(t, ok)
					assert.EqualValues(t, 21, attrVal.Int())
