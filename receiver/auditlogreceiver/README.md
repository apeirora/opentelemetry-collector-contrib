...
```bash
# PowerShell
Invoke-WebRequest -Uri "http://localhost:8080/auditlogs" -Method POST -ContentType "application/json" -Body '{"message": "Test audit log", "timestamp": "2024-01-01T00:00:00Z"}'

# Or with curl
curl -X POST http://localhost:8080/auditlogs \
...
1. **HTTP Handler**: Accepts audit log requests via POST to `/auditlogs`
...
- **Main Flow**: SDK â†’ Receiver â†’ Storage â†’ Processors â†’ Exporters â†’ Sinks
...
- **AuditLog Receiver**: `http://localhost:8080/auditlogs`
...
1. **Request Received**: HTTP POST request arrives at `/auditlogs`
...
Use Go **1.25.8+** (fixes a linker issue with `filterprocessor` in the full contrib build).

**Fast minimal build** (audit receiver, verify processor, file storage, debug only):

```bash
make otelauditcol
./bin/otelauditcol_windows_amd64.exe --config=receiver/auditlogreceiver/example-config.yaml
```

**Full contrib build** (all components, slow):
...
â”œâ”€â”€ auditlog-config.yaml      # Collector configuration
â”œâ”€â”€ .github/workflows/        # GitHub Actions
â””â”€â”€ storage/                  # Persistent storage directory
```
